...
export default function PurchaseOrders({ role, users }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Ensure all items have product and valid numbers
    ...
    <div className="page-enter">
      ...
      <div className="table-responsive-container" style={{ borderRadius: 14, border: `1px solid ${C.border}`, overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 900 }}>
          <THead cols={['PO ID', 'Date', 'Supplier', 'Items', 'Total Cost', 'Status', 'Actions']} />
          <tbody>
            {pos.length === 0 ? (
              <tr><td colSpan={7} style={{ padding: 0 }}><EmptyState title="No Purchase Orders" message="You have no purchase orders." icon="🚚" /></td></tr>
            ) : pos.map(po => (
              <TRow key={po._id}>
                <TCell bold>{po.poId}</TCell>
                <TCell>{new Date(po.createdAt).toLocaleDateString()}</TCell>
                <TCell>{po.supplier?.name || 'N/A'}</TCell>
                <TCell>
                ...
                <TCell bold>PKR {po.totalAmount.toLocaleString()}</TCell>
                <TCell><Badge s={po.status} /></TCell>
                <TCell>
      ...
      <ConfirmDialog 
        ...
        onCancel={() => setConfirmState({ isOpen: false, po: null })}
