                  ...
                  >
                    ...
                    {isSelected && (
                      <div onClick={e => e.stopPropagation()} style={ { display: 'flex', alignItems: 'center', gap: 6, marginTop: 8, borderTop: `1px solid ${C.goldBorder}`, paddingTop: 8 }}>
                        ...
                        <input type="number" min="0" value={selectedProducts.find(sp => sp.id === p.id).quantity} onChange={e => handleProductQuantityChange(p.id, parseInt(e.target.value) || 0)} style={ { flex: 1, minWidth: 0, padding: '4px', border: `1px solid ${C.goldBorder}`, borderRadius: 6, background: C.card, color: C.text, textAlign: 'center', fontSize: 13, outline: 'none' }} />
